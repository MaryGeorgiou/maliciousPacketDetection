...
class DataGenerator():
    ...
    def flow_from_directory(self):
        while True:
            ...
            for id in self.IDs:
                print(id)
                with open(self.data_dir + str(id) + 'kdd_indexed.csv', 'r') as csvfile:
                    X = []
                    Y = []
                    X = list(csv.reader(csvfile))
                    Y = keras.utils.to_categorical(np.load(self.data_dir + str(id) + 'labels.npy'), num_classes=self.n_classes)
                    print("\n File {} loaded.. Batching starts..".format(id)) 
                    b = 0 
                    inputs = np.empty(shape=())
                    targets = np.empty(shape=())
